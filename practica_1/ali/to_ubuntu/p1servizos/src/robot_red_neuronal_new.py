...
def alante(pub,move,vel):
    ...
    angulos.append(abs(move.angular.z))
    pub.publish(move)
...
def send_info(robot,W1,b1,fitness):
    ...
    robot_pesos="/".join([str(int(str(robot).split("_")[-1])+1),W1_todo_peso,b1_todo_peso,fitness,"\n"])
...
def funcion_fitness(robot,distancia_minimas,vels,angulos,tiempo,distancia_minim_front,choco):
    ...
    #print(robot,tiempo,desplacamiento,choco,np.mean(distancia_minimas),np.mean(angulos),np.mean(vels))
    ...
    vel=100*np.mean(vels)+(100/(np.mean(angulos)*10))+(100/(np.sum(angulos)))+(tiempo/(np.sum(angulos)*10))+100*np.min(vels)
...
def iniciar_nodo(argv):
    ...
    W1=[]
    b1=[]
    position_inicial=None

    try:
        opts, args = getopt.getopt(argv,":r:t:w:b:p:")
    ...
    #r1_positions.append([x,y])
    while (len(r1_dsf)==0 or len(r1_dsb)==0 or len(r1_positions)==0 ) or (limit!=None and ahora>=limit)  : 
        #print("esperando")
        a=1
        ahora = time.time()
    ...
    if len(r1_dsf)==0:
        ...
        #print("X: " ,X)
        #print(X.shape[0])
        if position_inicial==None:
            posicion_inicial=r1_positions[0]
        ...
        try:
            print("estamos entrando en el bucle principal",robot)
            while not rospy.is_shutdown():
                ...
                dist_minima_back.append(np.min(np.array(r1_dsb)))
                #print("X: " ,X)
                #print(r1_dsf)
                ...
                if dist_minima_front[-1]<=seguridad_dist or (limit!=None and ahora>=limit) or dist_minima[-1]<=seguridad_dist:
                        ...
                        reestar_robot_position(robot,posicion_inicial)
                        break
